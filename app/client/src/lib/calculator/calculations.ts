...
export function calculateAll(inputs: CalculatorInputs): CalculatorOutputs {
  ...
  const monthlyMortgage = calculateMonthlyMortgage(
    loanAmount,
    inputs.interestRateAnnualPercent,
  ...
  const noiMonthly =
    inputs.monthlyRent -
    (inputs.taxesMonthly + inputs.insuranceMonthly + inputs.otherFixedMonthly + vacancy + maintenance + management);
  ...
  const cashOnCashReturnPercent =
    totalCashInvested > 0 ? ((cashFlowMonthly * 12) / totalCashInvested) * 100 : 0;
  ...
  const annualizedFiveYearReturnPercent =
    totalCashInvested > 0 ? (Math.pow(1 + fiveYearProfit / totalCashInvested, 1 / 5) - 1) * 100 : 0;
